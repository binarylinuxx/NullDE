...
class HyprlandSettingsApp(Adw.Application):
    ...
    def get_memory_info(self):
        ...
        try:
            with open("/proc/meminfo") as f:
                meminfo = f.read()
            match_total = re.search(r'MemTotal:\s*(\d+)', meminfo)
            match_available = re.search(r'MemAvailable:\s*(\d+)', meminfo)
            if match_total and match_available:
                total = int(match_total.group(1)) // 1024
                available = int(match_available.group(1)) // 1024
    ...
    def do_activate(self):
        ...
        # Apply button (create before update_wallpaper_preview)
        ...
        stack.add_titled(appearance_box, "appearance", "Appearance")

        # Gaps In
        gaps_in_row = Adw.SpinRow.new_with_range(0, 50, 1)
        gaps_in_row.set_title("Gaps Between Windows (gaps_in)")
        gaps_in_row.set_value(8)
        gaps_in_row.connect("notify::value", self.on_gaps_in_changed)
        appearance_box.append(gaps_in_row)

        # Gaps Out
        gaps_out_row = Adw.SpinRow.new_with_range(0, 50, 1)
        gaps_out_row.set_title("Gaps Outer (gaps_out)")
        gaps_out_row.set_value(20)
        gaps_out_row.connect("notify::value", self.on_gaps_out_changed)
        appearance_box.append(gaps_out_row)

        # Border Size
        border_size_row = Adw.SpinRow.new_with_range(0, 20, 1)
        border_size_row.set_title("Border Size (border_size)")
        border_size_row.set_value(1)
        border_size_row.connect("notify::value", self.on_border_size_changed)
        appearance_box.append(border_size_row)

        # Rounding
        rounding_row = Adw.SpinRow.new_with_range(0, 50, 1)
        rounding_row.set_title("Corner Rounding (decoration:rounding)")
        rounding_row.set_value(0)
        rounding_row.connect("notify::value", self.on_rounding_changed)
        appearance_box.append(rounding_row)

        # Active Opacity
        active_opacity_row = Adw.SpinRow.new_with_range(0.1, 1.0, 0.01)
        active_opacity_row.set_title("Active Window Opacity (decoration:active_opacity)")
        active_opacity_row.set_value(1.0)
        active_opacity_row.connect("notify::value", self.on_active_opacity_changed)
        appearance_box.append(active_opacity_row)

        # Inactive Opacity
        inactive_opacity_row = Adw.SpinRow.new_with_range(0.1, 1.0, 0.01)
        inactive_opacity_row.set_title("Inactive Window Opacity (decoration:inactive_opacity)")
        inactive_opacity_row.set_value(1.0)
        inactive_opacity_row.connect("notify::value", self.on_inactive_opacity_changed)
        appearance_box.append(inactive_opacity_row)

        # Drop Shadow
        drop_shadow_switch = Gtk.Switch()
        drop_shadow_switch.set_active(True)
        drop_shadow_switch.connect("state-set", self.on_drop_shadow_changed)
        drop_shadow_row = Adw.ActionRow(title="Drop Shadow (decoration:drop_shadow)")
        drop_shadow_row.add_suffix(drop_shadow_switch)
        drop_shadow_row.set_activatable_widget(drop_shadow_switch)
        appearance_box.append(drop_shadow_row)

        # Animations Enable
        animations_switch = Gtk.Switch()
        animations_switch.set_active(True)
        animations_switch.connect("state-set", self.on_animations_enabled_changed)
        animations_row = Adw.ActionRow(title="Enable Animations (animations:enabled)")
        animations_row.add_suffix(animations_switch)
        animations_row.set_activatable_widget(animations_switch)
        appearance_box.append(animations_row)
    ...
    # --- Appearance Handlers ---
    def on_gaps_in_changed(self, spinrow, pspec):
        value = int(spinrow.get_value())
        subprocess.run(["hyprctl", "keyword", "general:gaps_in", str(value)])

    def on_gaps_out_changed(self, spinrow, pspec):
        value = int(spinrow.get_value())
        subprocess.run(["hyprctl", "keyword", "general:gaps_out", str(value)])

    def on_border_size_changed(self, spinrow, pspec):
        value = int(spinrow.get_value())
        subprocess.run(["hyprctl", "keyword", "general:border_size", str(value)])

    def on_rounding_changed(self, spinrow, pspec):
        value = int(spinrow.get_value())
        subprocess.run(["hyprctl", "keyword", "decoration:rounding", str(value)])

    def on_active_opacity_changed(self, spinrow, pspec):
        value = float(spinrow.get_value())
        subprocess.run(["hyprctl", "keyword", "decoration:active_opacity", str(value)])

    def on_inactive_opacity_changed(self, spinrow, pspec):
        value = float(spinrow.get_value())
        subprocess.run(["hyprctl", "keyword", "decoration:inactive_opacity", str(value)])

    def on_drop_shadow_changed(self, switch, state):
        subprocess.run(["hyprctl", "keyword", "decoration:drop_shadow", "true" if state else "false"])
        return False  # allow state to update

    def on_animations_enabled_changed(self, switch, state):
        subprocess.run(["hyprctl", "keyword", "animations:enabled", "true" if state else "false"])
        return False
