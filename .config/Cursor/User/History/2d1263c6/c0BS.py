...
class HyprlandSettingsApp(Adw.Application):
    ...
    def do_activate(self):
        ...
        appearance_frame = Adw.PreferencesGroup(title="Hyprland Appearance")
        appearance_box.append(appearance_frame)

        # Helper to get hyprctl option value
        def get_hypr_option(option):
            if shutil.which("hyprctl"):
                try:
                    result = subprocess.run(["hyprctl", "getoption", option], capture_output=True, text=True, check=True)
                    # Output format: option -> value (type)
        ...
        # Helper to set hyprctl option value
        def set_hypr_option(option, value):
            if shutil.which("hyprctl"):
                try:
                    subprocess.run(["hyprctl", "setoption", option, str(value)], check=True)
                    return True
                except Exception as e:
                    return False
            return False
        ...
        # Border size
        border_row = Adw.SpinRow.new_with_range(0, 20, 1)
        border_row.set_title("Border Size (px)")
        border_val = get_hypr_option("general:border_size")
        if border_val is not None:
            border_row.set_value(border_val)
        border_row.connect("value-changed", lambda row: set_hypr_option("general:border_size", int(row.get_value())))
        appearance_frame.add(border_row)

        # Rounding
        rounding_row = Adw.SpinRow.new_with_range(0, 30, 1)
        rounding_row.set_title("Corner Rounding (px)")
        rounding_val = get_hypr_option("general:rounding")
        if rounding_val is not None:
            rounding_row.set_value(rounding_val)
        rounding_row.connect("value-changed", lambda row: set_hypr_option("general:rounding", int(row.get_value())))
        appearance_frame.add(rounding_row)

        # Gaps
        gaps_row = Adw.SpinRow.new_with_range(0, 60, 1)
        gaps_row.set_title("Gaps (px)")
        gaps_val = get_hypr_option("general:gaps_in")
        if gaps_val is not None:
            gaps_row.set_value(gaps_val)
        gaps_row.connect("value-changed", lambda row: set_hypr_option("general:gaps_in", int(row.get_value())))
        appearance_frame.add(gaps_row)

        # Animation speed
        anim_row = Adw.SpinRow.new_with_range(0.01, 1.0, 0.01)
        anim_row.set_title("Animation Speed (s)")
        anim_val = get_hypr_option("animations:animation_speed")
        if anim_val is not None:
            anim_row.set_value(anim_val)
        anim_row.connect("value-changed", lambda row: set_hypr_option("animations:animation_speed", float(row.get_value())))
        appearance_frame.add(anim_row)

        # Error label if hyprctl is not found
        if not shutil.which("hyprctl"):
            error_label = Gtk.Label(label="hyprctl not found in PATH. Appearance controls are disabled.")
            error_label.set_halign(Gtk.Align.START)
            appearance_frame.add(error_label)

        # Create a scrolled window and set the main box as its child (restore this block)
