...
public:
    static std::vector<Color> extractColors(const std::string& imagePath) {
        ...
        std::string command = "magick \"" + imagePath + "\" +dither -colors 16 -format \"%c\" histogram:info: > " + tempFile;
...
public:
    ...
    bool processTemplate(const std::string& templatePath, const std::string& outputPath) {
        ...
        std::regex variableRegex(R"(\{\{color\.(\w+)\.(?:(\d+)|fg|bg)(?:\s*\|\s*(\w+(?:_\d+)?))?\}\})");
