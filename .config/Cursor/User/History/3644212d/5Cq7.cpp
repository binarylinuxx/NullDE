...
struct Color {
    ...
    // Ensure color is bright enough for display (minimum brightness threshold)
    Color ensureMinimumBrightness(int minBrightness = 60) const {
        ...
        double scale = (double)minBrightness / brightness;
        int newR = std::min(255, (int)(r * scale));
        int newG = std::min(255, (int)(g * scale));
        int newB = std::min(255, (int)(b * scale));
        return Color(newR, newG, newB);
    }
...
public:
    static std::vector<Color> extractColors(const std::string& imagePath) {
        ...
        // Use a more sophisticated approach to get better color distribution
        std::string command = "magick \"" + imagePath + "\" -resize 400x400 -quantize RGB -colors 48 -format \"%c\" histogram:info: | sort -k1 -nr > " + tempFile;
        ...
        while (std::getline(file, line)) {
            ...
            if (std::regex_search(line, match, colorRegex)) {
                ...
                allColors.emplace_back(r, g, b);
            }
        }
        
        // Sort colors by brightness and ensure good distribution
        std::sort(allColors.begin(), allColors.end(), 
                 [](const Color& a, const Color& b) { return a.getBrightness() < b.getBrightness(); });
        
        std::vector<Color> colors;
        // Take colors from different brightness ranges to ensure good distribution
        if (allColors.size() >= 16) {
            // Take 2 darkest, 2 lightest, and 12 from the middle
            colors.push_back(allColors[0].ensureMinimumBrightness(50));  // Darkest but not too dark
            colors.push_back(allColors[1].ensureMinimumBrightness(50));
            
            // Take colors from middle ranges
            for (int i = 2; i < 14; i++) {
                size_t idx = 2 + (i - 2) * (allColors.size() - 4) / 12;
                colors.push_back(allColors[idx].ensureMinimumBrightness(50));
            }
            
            // Take 2 lightest
            colors.push_back(allColors[allColors.size() - 2]);
            colors.push_back(allColors[allColors.size() - 1]);
        } else {
            // If we don't have enough colors, use what we have
            for (const auto& color : allColors) {
                colors.push_back(color.ensureMinimumBrightness(50));
            }
        }
        ...
        while (colors.size() < 16) {
            Color baseColor = colors.empty() ? Color(128, 128, 128) : colors[0];
            Color variant = generateColorVariant(baseColor, colors.size());
            // Ensure the variant is bright enough
            colors.push_back(variant.ensureMinimumBrightness(50));
        }
        ...
        for (size_t i = 0; i < colors.size(); ++i) {
            ...
            colors[i].display();
            std::cout << "        Brightness: " << colors[i].getBrightness() << std::endl;
        }
        std::cout << std::endl;
