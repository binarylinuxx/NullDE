...
struct Color {
    ...
    Color ensureMinimumBrightness(int minBrightness = 40) const {
        int brightness = (r + g + b) / 3;
        if (brightness >= minBrightness) return *this;
        double scale = (double)minBrightness / std::max(1, brightness);
...
public:
    static std::vector<Color> extractColors(const std::string& imagePath) {
        ...
        // Extract 8 main colors (like pywal)
        std::string command = "magick \"" + imagePath + "\" -resize 200x200 +dither -colors 8 -format \"%c\" histogram:info: > " + tempFile;
        ...
        while (std::getline(file, line) && baseColors.size() < 8) {
        ...
        // Ensure all base colors are at least min brightness
        for (auto& color : baseColors) {
            color = color.ensureMinimumBrightness(100);
        }
        
        // Add brighten function for pywal-style bright colors
        auto brighten = [](const Color& c, double factor = 0.4) -> Color {
            int newR = std::min(255, int(c.r + (255 - c.r) * factor));
            int newG = std::min(255, int(c.g + (255 - c.g) * factor));
            int newB = std::min(255, int(c.b + (255 - c.b) * factor));
            return Color(newR, newG, newB);
        };
        
        // If less than 8, fill with fallback
        while (baseColors.size() < 8) {
            baseColors.push_back(Color(128, 128, 128));
        }
        
        // Build pywal-style palette: 0-7 base, 8-15 bright
        std::vector<Color> palette = baseColors;
        for (const auto& color : baseColors) {
            palette.push_back(brighten(color));
        }
        
        std::cout << "\n[+] Extracted " << palette.size() << " colors (pywal/16 order):\n";
