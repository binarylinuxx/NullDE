...
public:
    static std::vector<Color> extractColors(const std::string& imagePath) {
        ...
        std::string command = "magick \"" + imagePath + "\" -resize 300x300 -quantize RGB -colors 32 -format \"%c\" histogram:info: | sort -k1 -nr > " + tempFile;
        ...
        if (allColors.size() >= 16) {
            ...
            colors.push_back(allColors[0].ensureMinimumBrightness(40));  // Darkest but not too dark
            colors.push_back(allColors[1].ensureMinimumBrightness(40));
            
            // Take colors from middle ranges
            for (int i = 2; i < 14; i++) {
                size_t idx = 2 + (i - 2) * (allColors.size() - 4) / 12;
                colors.push_back(allColors[idx]);
            }
        ...
        while (colors.size() < 16) {
            ...
            colors.push_back(variant.ensureMinimumBrightness(40));
        }
