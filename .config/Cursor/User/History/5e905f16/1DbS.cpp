...
public:
    ...
    void run(const std::string& filename) {
        try {
            ...
            // Парсим функции
            std::regex funcHeaderRegex(R"((func\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*\(\)\s*\{?)|(main\s*\(\)\s*\{?))");
            for (size_t i = 0; i < lines.size(); i++) {
                const std::string& line = lines[i];
                std::smatch match;
                if (std::regex_match(line, match, funcHeaderRegex)) {
                    std::string funcName;
                    if (match[2].matched) {
                    ...
                    if (line.back() == '{') {
                        i++; // body starts at next line
                    } else {
                        // Skip lines until we find a line that's just '{'
                        while (i + 1 < lines.size() && lines[i + 1] != "{") i++;
                        i += 2; // skip '{' line
                    }
                    // Read body until '}'
